...
async function test(){
    ...
    let link = 'https://www.rahmathpublications.com/bukharinew.php'
    ...
    fs.writeFileSync(path.join(__dirname,'bukharitamil.txt'),tamilbigArr.join('\n').trim())
    fs.writeFileSync(path.join(__dirname,'bukhariarabic.txt'),arabicbigArr.join('\n').trim())
    await browser.close()
}
