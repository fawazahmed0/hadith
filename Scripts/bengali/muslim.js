...
async function test(){
    ...
    let pagelink = 'https://www.hadithbd.com/hadith/chapter/?book=22'
...
//break
  ...
  fs.writeFileSync(path.join(__dirname,'muslimeng.txt'),englishArr.join('\n').trim())
  fs.writeFileSync(path.join(__dirname,'muslimbang.txt'),banglaArr.join('\n').trim())
